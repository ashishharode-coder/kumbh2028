                ...
                {!selectedStay ? (
                    ...
                    >
                        <header className="pt-24 pb-6 px-6 text-center bg-white">
                            ...
                            <h1 className="text-3xl md:text-5xl font-black italic tracking-tight">Pilgrim <span className="text-orange-600">Stay</span></h1>
                        </header>
                        ...
                        <div className="sticky top-[70px] md:top-[85px] z-40 bg-white/95 backdrop-blur-md border-b border-orange-100 py-1 md:py-4 shadow-sm transition-all">
                            <div className="max-w-7xl mx-auto space-y-3 px-4">
                                ...
                                <div className="flex flex-wrap items-center justify-center gap-2">
                                    {[{ id: 'low', label: 'Price: Low to High' }, { id: 'high', label: 'Price: High to Low' }].map(opt => (
                                        <button
                                            key={opt.id}
                                            ...
                                            className={`px-3 py-1.5 md:px-4 md:py-2 rounded-xl text-[9px] md:text-[10px] font-black border transition-all ${
                                                sortOrder === opt.id ? 'bg-orange-50 border-orange-600 text-orange-600 shadow-sm' : 'bg-transparent border-gray-100 text-gray-400'
                                            }`}
                                        >
                                            {opt.label}
                                        </button>
                        ...
                        <main className="max-w-7xl mx-auto px-4 py-8">
                            ...
                            {paginatedData.length === 0 ? (
                                ...
                                >
                                    <div className="relative mb-6">
                                        ...
                                        <span className="relative text-6xl md:text-8xl"><IoHome /></span>
                                    </div>
                                    ...
                                    <p className="text-gray-400 font-bold text-xs uppercase tracking-[0.2em] max-w-xs">
                                        We are currently arranging the best {activeTab} accommodations for the Mahakumbh.
                                    </p>
                                    <button 
                            ...
                            {totalPages > 1 && paginatedData.length > 0 && (
                                <div className="mt-10 flex justify-center items-center gap-2">
                                    <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="w-10 h-10 rounded-xl border border-orange-100 flex items-center justify-center disabled:opacity-20 bg-white shadow-sm">←</button>
                                    <div className="flex gap-1.5">
                                        {[...Array(totalPages)].map((_, i) => (
                                            <button key={i} onClick={() => setCurrentPage(i + 1)} className={`w-10 h-10 rounded-xl text-[10px] font-black transition-all ${currentPage === i + 1 ? 'bg-orange-600 text-white shadow-lg scale-110' : 'bg-white border border-orange-100 text-orange-900/40'}`}>{i + 1}</button>
                                        ))}
                                    </div>
                                    <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="w-10 h-10 rounded-xl border border-orange-100 flex items-center justify-center disabled:opacity-20 bg-white shadow-sm">→</button>
                                </div>
                            )}
                        </main>
                    </motion.div>
